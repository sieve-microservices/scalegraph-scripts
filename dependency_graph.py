...
EXCLUDE=["mongodb", "redis", "haproxy", "postgresql"]
...
def build_graph(gexf_path, dot_path, paths, significance=0.05):
    g = nx.MultiDiGraph()
    for path in paths:
        df = pd.read_csv(path, sep="\t")
        # bug in causation!
        ...
        while True:
            ...
            if row_a.p_for_lag_1 < significance:
                weight = (1-row_a.p_for_lag_1) * 10 - 9
                g.add_edge(service_a, service_b, name="%s -> %s" % (metric_a, metric_b), weight=weight)
            if row_b.p_for_lag_1 < significance:
                weight = (1-row_b.p_for_lag_1) * 10 - 9
                g.add_edge(service_b, service_a, name="%s -> %s" % (metric_b, metric_a), weight=weight)
